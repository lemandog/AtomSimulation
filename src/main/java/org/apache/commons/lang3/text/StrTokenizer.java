    ...
    private int readNextToken(final char[] srcChars, int start, final int len, final StrBuilder workArea, final List<String> tokenList) {
        ...
        while (start < len) {
            final int removeLen = Math.max(
                    getIgnoredMatcher().isMatch(srcChars, start, len),
                    getTrimmerMatcher().isMatch(srcChars, start, len));
            if (removeLen == 0 ||
                getDelimiterMatcher().isMatch(srcChars, start, len) > 0 ||
                getQuoteMatcher().isMatch(srcChars, start, len) > 0) {
                break;
            }
            start += removeLen;
        }
        ...
        // handle empty token
        final int delimLen = getDelimiterMatcher().isMatch(srcChars, start, len);
        if (delimLen > 0) {
            addToken(tokenList, StringUtils.EMPTY);
        ...
        // handle found token
        final int quoteLen = getQuoteMatcher().isMatch(srcChars, start, len);
        if (quoteLen > 0) {
    ...
    private int readWithQuotes(final char[] srcChars, final int start, final int len, final StrBuilder workArea,
        ...
        while (pos < len) {
            ...
            if (quoting) {
                ...
                final int delimLen = getDelimiterMatcher().isMatch(srcChars, pos, len);
                if (delimLen > 0) {
                ...
                final int ignoredLen = getIgnoredMatcher().isMatch(srcChars, pos, len);
                if (ignoredLen > 0) {
                    pos += ignoredLen;
                    continue;
                }
                ...
                final int trimmedLen = getTrimmerMatcher().isMatch(srcChars, pos, len);
                if (trimmedLen > 0) {
